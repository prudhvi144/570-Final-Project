...
class NavigationFunction2D:
    ...
    def calculate_path(self, start: np.ndarray, max_steps: int = 100, epsilon: float = None, scale_magnitude: float = None) -> np.ndarray:
    ...
    def plot_path_on_contour(self, start: np.ndarray, **kwargs):
        ...
        circle2 = plt.Circle((0, 0), 6, color='b', fill=False)
...
if __name__ == '__main__':
    obstacles_ = [
        Circle(np.array([0, 0]), 4),

    ]
    workspace = Circle(np.array([0., 0.]), 6)
    goal = np.array([5., 0.])
    ...
    navigation_function.plot_path_on_contour(np.array([[-4, .1]]), epsilon=1, scale_magnitude=.1)
    # navigation_function.plot_path_on_contour(np.array([-9, 0.1]), epsilon=1, scale_magnitude=.1)
    navigation_function.plot_navigation_surface(np.arange(-7, 7, .05), np.arange(-7, 7, .05))
    navigation_function.plot_navigation_contour(np.arange(-7, 7, .05), np.arange(-7, 7, .05))
    plt.show()
